...
class CalculatorNotifier extends StateNotifier<Calculator> {
  ...
  void append(String value) {
    ...
    if (!state.shouldAppend) {
      if (!RegExp(r'(?<=[\d\)])[-×÷+*/]').hasMatch(value)) {
        state = state.copy(
            equation: state.result + value, result: "", shouldAppend: true);
      } else {
        print("f");
      ...
      if (equation.isNotEmpty && equation != "0") {
        ...
        if (RegExp(r'\d').hasMatch(lastChar)) {
          final lastNumberMatch =
              RegExp(r'(\(?-?\d+(\.\d+)?\)?)$').firstMatch(equation);
          if (lastNumberMatch != null) {
            ...
            final newNumber = lastNumber.startsWith('-')
                ...
                : '(-$lastNumber)';
            final newEquation =
                equation.substring(0, lastNumberMatch.start) + newNumber;
  ...
  void calculate() {
    ...
    if (equation.isNotEmpty) {
      ...
      state =
          state.copy(result: calculateResult.toString(), shouldAppend: false);
    }
  }
  ...
  void memoryRecall() {
    final equation = state.equation;
    final memory =
        double.parse(state.memory) < 0.0 ? "(${state.memory})" : state.memory;
    final lastChar = equation[equation.length - 1];
    if (equation == "0") {
  ...
  void memoryAdd() {
    state = state.copy(
        memory: (double.parse(state.memory) + double.parse(state.result))
            .toString());
  }
